/**
 * YouTube Fetch Script - Vietnamese Shorts
 *
 * Fetches YouTube Shorts from Vietnamese tech channels
 * Shorts are already in Vietnamese - no translation needed
 *
 * Usage: node scripts/fetch-youtube-vn-shorts.js
 */
...
function getVideoThumbnail(videoId) {
  // Try maxresdefault first, frontend can fallback to hqdefault
 ...
 * Note: YouTube RSS doesn't mark Shorts explicitly, so we check video duration via API or title patterns
 * For now, we'll fetch all and let n8n filter by duration later
 */
function parseYouTubeRSS(xmlText, shortsOnly = true) {
  const items = []
  ...
  while ((match = entryRegex.exec(xmlText)) !== null) {
    ...
    if (videoId && title && link) {
      // For Shorts, we'll include all videos and mark them
      // The actual filtering can be done by checking title patterns or video duration
      const isLikelyShort =
...
/**
 * Insert video - Vietnamese Shorts go directly to public
 * Since it's already Vietnamese, no translation needed
 */
async function insertVideo(creatorId, video) {
  ...
  const { data: post, error } = await supabase
    ...
    .insert({
      ...
      title: video.title,
      title_vi: video.title,  // Same as title since it's Vietnamese
...
async function fetchChannel(channel) {
  ...
  try {
    ...
    const videos = parseYouTubeRSS(xmlText, false) // Get all videos for now
...
async function main() {
  console.log('🎬 YouTube Fetcher - Vietnamese Shorts')
